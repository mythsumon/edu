# Documentation Index

Welcome to the Education Dashboard documentation. This folder contains guides and resources to help you understand, use, and contribute to the project.

## Getting Started
- [Main README](../README.md) - Quick start guide and project overview
- [User Guide](../USER_GUIDE.md) - Comprehensive guide for using the dashboard

## Technical Documentation
- [Project Structure](../README.md#project-structure) - Overview of the codebase organization
- [Development Setup](../README.md#getting-started) - Instructions for setting up the development environment

## Feature Guides
- [Multilingual Support](../USER_GUIDE.md#language-support) - How to use and configure language options
- [Data Filtering](../USER_GUIDE.md#filtering-data) - How to filter and search data
- [Visualization Interaction](../USER_GUIDE.md#interacting-with-visualizations) - How to interact with charts and maps

## Troubleshooting
- [Common Issues](../USER_GUIDE.md#troubleshooting) - Solutions to frequent problems